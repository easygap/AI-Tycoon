...
- `server.js` 는 기본적으로 `127.0.0.1` (루프백) 에만 바인딩한다 — 같은 네트워크의
  다른 기기에서 프롬프트·프로젝트명이 노출되는 걸 막기 위함. 외부 노출이 필요하면
  `HOST` 환경변수로 직접 opt-in 하고, 리버스 프록시 + 인증을 함께 두는 걸 권장한다.
  *(v1.5.0 부터)*
- WebSocket 은 연결 시 `Origin` 헤더를 검증해 루프백 출처만 허용한다. 사용자가 방문한
  악성 웹페이지가 `ws://localhost` 로 몰래 붙어 작업 내용(프롬프트 포함)을 가로채는
  *Cross-Site WebSocket Hijacking* 을 막는다. `HOST` 로 외부 노출을 opt-in 하면 이 검증은
  완화된다 (사용자 책임). *(v1.5.0 부터)*
- `/api/agents` 는 CORS 와일드카드(`*`) 를 보내지 않는다 — 임의 웹페이지의 cross-origin
  fetch 로 로컬 에이전트 현황이 새는 걸 막기 위함. 같은 출처(대시보드)와 CLI/스크립트
  (curl 등, CORS 무관) 는 정상 동작한다. *(v1.5.0 부터)*
