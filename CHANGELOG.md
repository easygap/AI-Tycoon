...
- 2026년 색상·패션 자료를 확인하고 하늘색, 자두색, 주황색을 화면과 캐릭터 코디에 적용했습니다. 원문 출처와 자체 해석을 디자인 문서에 구분해 적었습니다.
- 캐릭터를 32×40 픽셀로 다시 그리고 옷깃·소매·가방·신발이 다른 여섯 코디를 만들었습니다. 직원 상세에서 옷을 고르고 네 방향으로 돌려 볼 수 있습니다.
- 옷을 바꿔도 얼굴과 이름은 유지됩니다. 같은 도구의 세션에 코디를 저장하고, 체험 직원과 실제 직원은 따로 관리합니다.
- 직원 상세에 ‘직원 목록’과 ‘맵에서 보기’를 추가했습니다. 목록으로 돌아오면 원래 직원 카드에 키보드 포커스를 돌려줍니다.
- 직원 화면을 보는 중에는 모바일 출퇴근·완료 토스트를 생략하고, 확인 요청은 하단에 표시합니다. 알림을 키보드로도 열 수 있으며 해당 직원 상세로 바로 이동합니다.
- 밝은 옷을 골랐을 때 작업 내용까지 흐려지던 글자색과, 옷장을 연 상태에서 언어를 바꿀 때 표시가 갱신되지 않던 문제를 고쳤습니다.
- 모바일에서 테마를 바꿔도 상단 높이가 유지되도록 맞췄습니다. 직원 이동 버튼, 통계의 이름 표시와 Ollama 막대도 밝기와 대비를 보완했습니다.
- 코디 저장·복원·입력 중 갱신·모바일 화면을 검증하고 README의 실제 캡처를 갱신했습니다.

## 1.7.0 — 2026-09-22

- 사무실 맵을 둥근 회의 자리, 작업실, 유리 지붕 자료실, 카페 테라스가 이어지는 작은 제작소로 바꿨습니다. 연결 통로와 돌출된 입구, 계단으로 외곽 형태도 새로 잡았습니다.
- ‘전체 맵’ 메뉴에서 공간을 골라 확대할 수 있습니다. 화면을 회전해도 고른 공간에 다시 맞추며, 확대된 맵이 제목과 상태 정보를 덮지 않습니다.
- 직원과 청소 로봇·강아지·야간 순찰·배달원이 같은 바닥 정보를 사용합니다. 가구와 빈 마당을 피해서 이동하도록 고쳤습니다.
- 직원이 교체될 때 자리가 겹치던 문제를 수정했습니다. 사용 중인 자리를 유지하면서 빈자리를 배정합니다.
- 맵과 맞지 않던 예전 장식 효과를 제거하고, 건물·가구 그림을 재사용하도록 정리했습니다. 창밖의 낮과 밤, 오프라인 실행, 동작 줄이기를 유지합니다.
- 이동 경로와 충돌 검사를 추가하고 실제 맵 캡처와 디자인 참고 자료를 갱신했습니다.

## 1.6.0 — 2026-09-22

- 작은 회사가 중심이 되도록 화면을 다시 구성했습니다. 모바일에서는 회사·직원·기록을 하단 메뉴로 오갈 수 있습니다.
- Wanted Sans와 Galmuri를 적용하고, 긴 이름과 작업명이 잘리지 않도록 직원 목록을 고쳤습니다.
- 캐릭터의 얼굴·머리·소품과 네 방향 이동, 타이핑·검색·확인 자세를 새로 그렸습니다. 방에는 출입구, 햇빛, 배선과 그림자를 더했습니다.
- 모든 직원에게 뜨던 말풍선과 겹치는 효과를 줄였습니다. 동작 줄이기 설정도 반영합니다.
- 직접 만든 배경음악과 상황별 효과음을 추가했습니다. 소리는 선택해서 켜며, 겹치는 알림은 줄이고 다른 탭에서는 재생을 멈춥니다.
- 체험 모드와 실제 작업의 화면·통계·업적을 분리했습니다. 체험을 끄면 최근 실제 상태로 돌아옵니다.
- 글꼴과 화면 라이브러리를 앱에 포함하고, 오프라인 캐시의 누락·오류 처리를 고쳤습니다.
- 첫 실행 안내를 복구하고, 실제 앱 캡처로 README를 새로 작성했습니다.
